...
function GlowMatchApp() {
  ...
  useEffect(() => {
    const loadModels = async () => {
      try {
        ...
        console.error('⚠️ Failed to load models:', err);
      }
    };
    loadModels();
  }, []);
  ...
  useEffect(() => {
    ...
    const detectFace = async () => {
      ...
      if (detection) {
        ...
        setAuraMatch(getAuraName(expression));
  ...
  return (
    <div style={{ textAlign: 'center' }} onTouchMove={handleTouchMove}>
      ...
      <video ref={videoRef} autoPlay muted width="320" height="240" style={{ display: uploadedImage ? 'none' : 'block' }} />
      <canvas ref={canvasRef} width="640" height="480" />
      <div>
        {savedLooks.map((look, index) => (
          <img key={index} src={look} alt="Saved look" width="100" height="100" />
        ))}
      </div>
    </div>
  );
